...
#     -scenario all `
#     -msiPath ".\contrib\win-installer\podman-5.4.0-dev-setup.exe" `
#     -provider hyperv
# .\contrib\win-installer\test-installer.ps1 `
#     -scenario installation-green-field `
#     -msiPath ".\contrib\win-installer\en-US\podman.msi"
#     -provider hyperv
#     -allusers
# .\contrib\win-installer\test-installer.ps1 `
#     -scenario update-from-current-to-next `
#     -previousSetupExePath podman-5.5.2-setup.exe `
#     -msiPath "en-US\podman.msi"
...
function Install-Podman-Bundle {
    ...
    if ($script:skipWinVersionCheck) {$allowOldWinVar = "1"} else {$allowOldWinVar = "0"}
    ...
    Write-Host "(provider=`"$script:provider`", AllowOldWin=`"$allowOldWinVar`", SkipConfigFileCreation=`"$skipConfigFileCreationVar`")"
